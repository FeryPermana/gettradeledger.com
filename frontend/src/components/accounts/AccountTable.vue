    ...
    <table class="w-full text-sm">
      <thead class="surface-table-head">
        <tr>
          ...
          <th class="p-4 text-left">Initial Balance</th>
      ...
      <tbody>
        <tr v-for="item in items" :key="item.id" class="surface-table-row">
          ...
          <td class="p-4 page-body">{{ formatType(item.type) }}</td>
          <td class="p-4 page-body">{{ item.currency }}</td>
          <td class="p-4 page-body">
            {{ formatCurrency(item.initial_balance, item.currency) }}
          </td>
          <td class="p-4">
            <div class="flex justify-end gap-2">
              <button class="btn-soft rounded-lg px-3 py-1.5" @click="$emit('view', item.id)">
                View
              </button>
